...
export const Reviews: React.FC = () => {
  ...
  const scrollToReview = (index: number) => {
    if (scrollRef.current) {
      const { clientWidth } = scrollRef.current;
      // In mobile, cards are 100%, in desktop 80%. Simplification: scroll based on child width
      // Better approach for exact scroll snap index:
      const card = scrollRef.current.children[index] as HTMLElement;
      if (card) {
        // Center the card
        const scrollLeft = card.offsetLeft - (clientWidth - card.clientWidth) / 2;
  ...
  return (
    <section className="stage min-h-screen relative overflow-hidden scroll-snap-align-start flex flex-col justify-center py-20" id="stories">
      <img 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10 w-full">
        ...
        <div className="relative group max-w-6xl mx-auto">
          ...
          <div 
            ref={scrollRef}
            className="flex gap-8 overflow-x-auto pb-12 pt-4 px-4 scrollbar-hide snap-x snap-mandatory" 
            style={{ scrollBehavior: 'smooth' }}
          >
            {REVIEWS.map((review) => (
              <div key={review.id} className="snap-center flex-shrink-0 w-full md:w-[85%] lg:w-[80%]">
                <div className="bg-white rounded-[40px] p-8 md:p-10 text-left shadow-2xl border border-slate-50 flex flex-col md:flex-row gap-8 items-center h-full">
                  <div className="w-full md:w-64 h-80 shrink-0 rounded-3xl overflow-hidden shadow-inner">
                    <img 
                        src={review.image} 
                        alt={review.name} 
                        className="w-full h-full object-cover vivid-img" 
                    />
                  </div>
                    ...
                    <div>
                      <div className="flex gap-0.5 text-brand-red mb-4">
                        {[...Array(review.stars)].map((_, i) => (
                          <span key={i} className="material-symbols-outlined star-filled text-[20px]">star</span>
                        ))}
                      </div>
                      <p className="text-lg md:text-xl font-bold mb-6 italic text-charcoal leading-relaxed">"{review.text}"</p>
                    </div>
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-full bg-brand-red flex items-center justify-center text-white font-black text-xs">{review.initials}</div>
                      <div>
                        <div className="text-[11px] font-black tracking-widest uppercase text-charcoal">{review.name}</div>
          ...
          <div className="flex justify-center gap-3 mt-4">
            {REVIEWS.map((_, i) => (
              <button 
                ...
                className={`w-2.5 h-2.5 rounded-full transition-all ${i === activeIndex ? 'bg-brand-red' : 'bg-slate-300 hover:bg-brand-red/50'}`}
              />
            ))}
          </div>
        </div>
